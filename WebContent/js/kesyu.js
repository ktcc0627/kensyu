...
document.addEventListener('DOMContentLoaded', function() {
	...
	var targets = document.getElementsByClassName('company');
	  for (var i=0 ; i<targets.length ; i++) {
	    ...
	    targets[i].oninput = function () {
	      var alertelement = this.parentNode.getElementsByClassName('companyArea');
	...
	targets = document.getElementsByClassName('name');
	  for (var i=0 ; i<targets.length ; i++) {
	    // ▼文字が入力されたタイミングでチェックする：
	    targets[i].oninput = function () {
	      var alertelement = this.parentNode.getElementsByClassName('nameArea');
		...
		targets = document.getElementsByClassName('visitor');
		  for (var i=0 ; i<targets.length ; i++) {
		    // ▼文字が入力されたタイミングでチェックする：
		    targets[i].oninput = function () {
		      var alertelement = this.parentNode.getElementsByClassName('visitortArea');
		      if( this.value.trim().length > 10 ) {
		        // ▼空白を除いた入力文字数が10文字よりも多ければ
		        if( alertelement[0] ) { alertelement[0].innerHTML = "  <font size=\"3px\" color=\"red\"> ※10文字以下で入力して下さい。</font>"; }
		      }
...
//	      var alerts = this.getElementsByClassName('alertArea');
	      ...
	      const nameArea = document.getElementsByClassName('nameArea');
	      const companyArea = document.getElementsByClassName('companyArea');
	      const alertArea = document.getElementsByClassName('alertArea');
	      const visitorArea = document.getElementsByClassName('visitorArea');
	      const tenWord = "を10文字以下で入力してください。";
	      const thirtyWord = "を30文字以内で入力してください。";
	      const blank = "必須項目を入力してください。";


	      if((nameArea.value !== "" || nameArea.value !== null) && (companyArea.value !== "" || companyArea.value !== null) &&
	    		  (alertArea.value !== "" || alertArea.value !== null) && (visitorArea.value !== "" || visitorArea.value !== null)){
	    	  alert("aaaa");
	      }
...
//	      var alerts = [];
//	      alerts.push("message");
//	      var arrays = [tenWord, thirtyWord, blank];  //アラート内容の配列
//
